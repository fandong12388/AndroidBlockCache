 ...
 * description:
 */
public class CacheReadDoubleTask extends CacheReadTask<Double> {
    private static final Double DEFAULT_VALUE = 0.;

    public CacheReadDoubleTask(String block, String key, Double defaultValue) {
        super(block, key, defaultValue);
    }

    @Override
    public void postExecute(Cache<Double> cache) {
        if (null == mListener) {
            return;
        }
        String value = null;
        ...
        if (TextUtils.isEmpty(value)) {
            ...
            try {
                mListener.onFinish(Double.valueOf(value));
